...
3. Dispatch a DIFFERENT-vendor sub-agent as reviewer (Claude built it →
   `codex`, `pi`, or `antigravity`; Codex built it → `claude_code`, `pi`, or
   `antigravity`; Pi built it → `claude_code`, `codex`, or `antigravity`;
   Antigravity built it → `claude_code`, `codex`, or `pi`). Use a task-based
   title such as `review-auth-refactor`, never the raw vendor name:
   `sys_session_send(agent="claude_code"|"codex"|"pi"|"antigravity", title="review-<task_slug>",
...
- Review is a coding sub-agent (`claude_code`/`codex`/`pi`/`antigravity`)
  dispatched with
