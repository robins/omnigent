...
(claude_code / codex / pi / antigravity, which implement, review, and
explore), the three spine skills, and the bounds/blast-radius guardrails.
Pure spec-load — no LLM, no credentials.
...
def test_coding_subagents(polly_spec: AgentSpec) -> None:
    ...
    terminal harnesses, plus ``pi`` (Pi, pi) and ``antigravity`` (Antigravity,
    Google's Gemini-native SDK harness) as the headless workers. All implement,
    review, and explore. The native harnesses make claude_code / codex render
    ...
    assert sorted(polly_spec.tools.agents) == ["antigravity", "claude_code", "codex", "pi"]
    assert fam["claude_code"] == "claude-native"
    assert fam["codex"] == "codex-native"
    assert fam["pi"] == "pi"
    assert fam["antigravity"] == "antigravity"
    # Four distinct vendors → any diff is always reviewable by another.
    assert len(set(fam.values())) == 4
    for name in ("claude_code", "codex", "pi", "antigravity"):
...
def test_investigation_skill_delegates_read_only_work() -> None:
    ...
    assert (
        "Dispatch each task to `claude_code`, `codex`, `pi`, or `antigravity`: "
        '`sys_session_send(agent="claude_code"|"codex"|"pi"|"antigravity", '
...
def test_function_policies_have_nonempty_arguments(polly_spec: AgentSpec) -> None:
    ...
    # = 3; sub-agents: blast_radius x4 (claude_code, codex, pi, antigravity) = 4
